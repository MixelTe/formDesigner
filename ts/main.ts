...
addInputListener("inp_name", inp =>
{
	...
	{
		if (inp.value.length > 0) selectedRect.name = inp.value;
...
canvas.addEventListener("mousemove", e =>
{
	...
	else if (rectToSize != null)
	{
		...
		if (rectToSize.width != w || rectToSize.height != h) rectSizeChanged = true;
		rectToSize.width = w;
		rectToSize.height = h;
		drawAll();
	}
...
function setRect(rect: Rect | null)
{
	selectedRect = rect;
	if (rect == null)
	{
...
function addInputListener(id: string, f: (inp: HTMLInputElement) => void, onCreate?: (inp: HTMLInputElement) => void)
{
	const inp = document.getElementById(id);
	if (inp == undefined) throw new Error(`El not found: ${id}`);
	if (!(inp instanceof HTMLInputElement)) throw new Error(`El not input: ${id}`);
	inp.addEventListener("change", f.bind(undefined, inp));
	if (onCreate) onCreate(inp);
}
function addInputListenerField(id: string, field: "width" | "height" | "margin" | "cellSize")
{
	addInputListener(id,
		inp =>
		{
...
function addInputListenerRect(id: string, field: "width" | "height" | "x" | "y", min = 1)
{
	addInputListener(id,
		inp =>
		{
			...
			else if (field == "width") v = Math.min(v, form.width - selectedRect.x);
			else if (field == "height") v = Math.min(v, form.height - selectedRect.y);
			selectedRect[field] = v;
			inp.valueAsNumber = v;
			drawAll();
		});
}
function addRect(e: MouseEvent | null, x?: number, y?: number)
{
	const rect = new Rect();
	rects.push(rect);
	if (x) rect.x = x;
...
function removeRect()
{
	...
	setRect(null);
	if (rects.length == 0) rectsCreated = 0;
	drawAll();
}